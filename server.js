...
// Minimālais ātrums lai skaitītu kustību (km/h)
const MIN_SPEED_KMH = 3;
// Minimālā distance starp punktiem lai skaitītu (km) — ~10 metri
...
async function processGPS({ lat, lng, speed_kmh, heading, altitude }, res) {
  ...
  try {
    ...
    // Vai auto kustās? — ātrums > slieksnis UN distance > minimālā
    const isMoving = speed_kmh >= MIN_SPEED_KMH && distanceDelta >= MIN_DISTANCE_KM;

    // Insert point (vienmēr saglabā pozīciju)
    await pool.query(
    ...
    // Update trip stats — distanci un laiku skaita TIKAI ja kustās
    const tripRes = await pool.query('SELECT * FROM trips WHERE id = $1', [currentTripId]);
    const trip = tripRes.rows[0];

    const newDistance = (parseFloat(trip.distance_km) || 0) + (isMoving ? distanceDelta : 0);

    // Laiku skaita no iepriekšējā punkta — tikai ja kustās
    let addedSec = 0;
    if (isMoving && lastPoint) {
      addedSec = Math.floor((new Date(now) - new Date(lastPoint.recorded_at)) / 1000);
      // Nepieskaitīt vairāk par 5 minūtēm vienā solī (ja GPS pārtrūka)
      if (addedSec > 300) addedSec = 0;
    }
    const newDurationSec = (parseInt(trip.duration_sec) || 0) + addedSec;

    const avgSpeed = newDurationSec > 0 ? (newDistance / newDurationSec * 3600) : 0;
    ...
    await pool.query(
      ...
      [newDistance, newDurationSec, avgSpeed, maxSpeed, currentTripId]
    );
    ...
    broadcast({ type: 'trip_update', trip: { ...trip, distance_km: newDistance, duration_sec: newDurationSec, avg_speed_kmh: avgSpeed, max_speed_kmh: maxSpeed } });
