...
// --- Traccar Client endpoint (GET) ---
// Traccar sūta: /?id=device&lat=56.9&lon=24.1&speed=50&bearing=90&altitude=10
app.get('/', (req, res) => {
  const lat = parseFloat(req.query.lat);
  const lng = parseFloat(req.query.lon || req.query.lng);
  const speed_kmh = parseFloat(req.query.speed || 0);
  const heading = parseFloat(req.query.bearing || req.query.heading || 0);
  const altitude = parseFloat(req.query.altitude || 0);
 
  if (isNaN(lat) || isNaN(lng)) return res.status(400).send('Missing lat/lon');
  console.log(`Traccar GPS: ${lat}, ${lng}, ${speed_kmh} km/h`);
  processGPS({ lat, lng, speed_kmh, heading, altitude }, res);
});
 
// --- POST /api/gps endpoint ---
...
app.get('/api/stats', (req, res) => {
  const stats = db.prepare(`
    ...
    COALESCE(SUM(duration_sec),0) as total_sec, COALESCE(AVG(avg_speed_kmh),0) as overall_avg_speed,
